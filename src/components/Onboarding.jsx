...
export default function Onboarding({ onFinish }){
  ...
  return <div className="min-h-screen px-4 py-8"><div className="max-w-3xl mx-auto card p-6 md:p-8"><span className="badge">新手測驗</span><h1 className="text-3xl font-black text-bark mt-3">先了解你的生活情境</h1><p className="text-soil/75 mt-2">回答後，系統會分配你的自足學習路線。</p><div className="mt-6 space-y-6">{questions.map(([key,q,opts],idx)=><div key={key}><h2 className="font-bold text-bark mb-3">{idx+1}. {q}</h2><div className="grid sm:grid-cols-2 gap-2">{opts.map(opt=><button key={opt} onClick={()=>setAnswers({...answers,[key]:opt})} className={`text-left rounded-2xl px-4 py-3 border transition ${answers[key]===opt?'bg-moss text-white border-moss':'bg-white/70 border-soil/10 hover:bg-cream'}`}>{opt}</button>)}</div></div>)}</div><button disabled={!done} onClick={()=>onFinish(answers)} className={`mt-8 w-full ${done?'btn-primary':'px-5 py-3 rounded-2xl bg-soil/20 text-soil/50 cursor-not-allowed'}`}>完成測驗，進入自足村</button></div></div>
}
