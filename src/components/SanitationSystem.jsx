...
export default function SanitationSystem({ setPage }) {
  ...
  return <div className="sanitation-system space-y-5 pb-32">
    ...
    {Object.entries(lists).map(([key, config]) => <section key={key} className="muji-card"><div className="flex items-center justify-between gap-3"><Title>{config.title}</Title><button className="btn-secondary" onClick={() => begin(key)}><Plus size={16} className="inline"/> 新增</button></div>{editing.key === key && <ItemForm config={config} item={editing.item} setItem={(item) => setEditing({ key, item })} save={save} cancel={() => setEditing({ key: '', item: null })}/>}<div className="sanitation-card-grid">{data[key].map((item) => <article key={item.id} className={config.plan ? 'sanitation-plan-card' : 'sanitation-supply-card'}><div className="flex items-start justify-between gap-3"><div><h3>{item.name}</h3>{item.type && <p>{sanitationTypeLabels[item.type] || item.type}</p>}</div><div className="shrink-0"><button aria-label={`編輯${item.name}`} className="p-2" onClick={() => begin(key,item)}><Pencil size={16}/></button><button aria-label={`刪除${item.name}`} className="p-2 text-[#8b2f25]" onClick={() => remove(key,item)}><Trash2 size={16}/></button></div></div>{config.plan ? <PlanSummary item={item} summary={summary}/> : <p className="mt-2 text-sm font-bold">{config.fields.map(([field,label]) => `${label} ${fmt(item[field])}`).join(' · ')}</p>}{item.notes && <small>{item.notes}</small>}</article>)}</div>{!data[key].length && <p className="mt-3 rounded-xl bg-white/55 p-3 text-sm font-bold">尚未建立資料。</p>}</section>)}
