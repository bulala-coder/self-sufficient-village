...
export default function Roadmap({ state, updateRoadmap }) {
  ...
  const specialFood=Number(food.data.household.children)+Number(food.data.household.seniors)+Number(food.data.household.specialNeeds),foodLevels=[{level:'Level 1：72 小時家庭食物',items:[['建立 72 小時食物熱量',food.days.foodDays>=3],['建立免烹調食物',food.days.readyToEatDays>=3],['建立低用水食物',food.capabilities.lowWaterItemCount>0],['建立寵物 72 小時食物',food.data.household.pets<=0||food.days.petFoodDays>=3],['建立基本配給方案',food.capabilities.rationPlanCount>0]]},{level:'Level 2：7 天補給中斷',items:[['建立 7 天食物支撐',food.days.foodDays>=7],['建立 7 天寵物食物',food.data.household.pets<=0||food.days.petFoodDays>=7],['建立烹調方案',food.capabilities.cookingPlanCount>0],['建立保存期限檢查制度',food.capabilities.expiringItemCount===0&&food.capabilities.foodItemCount>0],['建立特殊需求食物',specialFood<=0||food.data.foodItems.some((item)=>['babyFood','seniorFood','specialDiet'].includes(item?.type))]]},{level:'Level 3：長期食物韌性',items:[['建立 14 天食物壓力測試',food.days.overallDays>=14],['建立低水、低能源烹調策略',food.capabilities.lowWaterItemCount>0&&food.data.cookingPlans.some((item)=>item?.energyEfficient)],['建立輪替與補貨制度',food.capabilities.expiringItemCount===0&&food.data.foodItems.some((item)=>item?.notes)],['建立家庭食物分配角色',food.data.rationPlans.some((item)=>item?.notes)]]}]
  const communicationLevels=[{level:'Level 1：72 小時基本通訊',items:[['建立紙本緊急聯絡清單',communication.capabilities.hasPrintedContactList],['建立家人、鄰居、醫療與獸醫聯絡人',communication.capabilities.hasFamilyContact&&communication.capabilities.hasNeighborContact&&communication.capabilities.hasMedicalContact&&(communication.data.household.pets<=0||communication.capabilities.hasVetContact)],['建立離線地圖',communication.capabilities.hasOfflineMap],['建立家庭集合地點',communication.totals.meetingPointCount>0],['建立手機充電與充電線備援',communication.data.devices.some((i)=>['powerCable','powerAdapter'].includes(i?.type))]]},{level:'Level 2：7 天斷網與停電',items:[['建立收音機或離線資訊來源',communication.capabilities.hasRadio||communication.totals.informationSourceCount>0],['建立不依賴網路的通訊方式',communication.capabilities.hasRadio||communication.capabilities.hasWalkieTalkie],['建立外地聯絡人',communication.capabilities.hasOutOfAreaContact],['建立家庭報平安計畫',communication.totals.messagePlanCount>0],['建立寵物與特殊需求者資訊備份',communication.capabilities.hasPetPlan&&communication.readiness.documentReadiness>0]]},{level:'Level 3：重大災害通訊備援',items:[['建立多層集合地點',communication.totals.meetingPointCount>=2],['建立紙本醫療、寵物與物資資料',communication.data.offlineDocuments.filter((i)=>['medicalInfo','petInfo','supplyList'].includes(i?.type)&&i?.hasPrintedCopy).length>=2],['建立社區互助聯絡網',communication.data.contacts.some((i)=>i?.role==='community')],['建立通訊中斷演練流程',communication.data.messagePlans.some((i)=>i?.notes)]]}]
  const roadmap = state.roadmap || {}
  const checks = roadmap.checks || {}
  ...
  return (
    <div className="roadmap-page space-y-5 pb-32">
      ...
      <section className="muji-card food-summary-card"><div className="muji-section-title"><Target size={18}/><span>Food System Roadmap｜食物韌性路線</span></div><div className="mt-4 grid gap-4 lg:grid-cols-3">{foodLevels.map((group)=><article key={group.level} className="food-roadmap-card"><div className="flex items-start justify-between gap-2"><h3>{group.level}</h3><span className="badge">{food.status}</span></div><div className="mt-3 space-y-2">{group.items.map(([label,complete])=>{const priority=!complete&&food.score<30,status=complete?'已完成':priority?'優先處理':'需要加強',statusClass=complete?'bg-[#24483a] text-[#fff9ea]':priority?'bg-[#8b2f25] text-[#fff9ea]':'bg-[#c2a25c] text-[#241b10]';return <div key={label} className="roadmap-card"><span>{label}</span><span className={`status-pill ${statusClass}`}>{status}</span></div>})}</div></article>)}</div></section>

      <section className="muji-card communication-summary-card"><div className="muji-section-title"><Target size={18}/><span>Communication System Roadmap｜通訊韌性路線</span></div><div className="mt-4 grid gap-4 lg:grid-cols-3">{communicationLevels.map((group)=><article key={group.level} className="communication-roadmap-card"><div className="flex items-start justify-between gap-2"><h3>{group.level}</h3><span className="badge">{communication.status}</span></div><div className="mt-3 space-y-2">{group.items.map(([label,complete])=>{const priority=!complete&&communication.score<30,status=complete?'已完成':priority?'優先處理':'需要加強',statusClass=complete?'bg-[#24483a] text-[#fff9ea]':priority?'bg-[#8b2f25] text-[#fff9ea]':'bg-[#c2a25c] text-[#241b10]';return <div key={label} className="roadmap-card"><span>{label}</span><span className={`status-pill ${statusClass}`}>{status}</span></div>})}</div></article>)}</div></section>
