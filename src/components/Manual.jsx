...
const manualSections = [
  ...
  },
  {
    ...
    immediateActions: ['先確認人員與動物安全', '開啟最低照明，不要浪費電力', '手機改低耗電模式', '冰箱冷凍庫非必要不要打開', '盤點行動電源與電池', '安全時執行 30 分鐘黑暗演練，確認動線與照明位置', '藥品冷藏優先於娛樂用電'],
    doNotDo: ['不要在密閉空間使用燃油發電機或炭火', '不要頻繁打開冰箱', '不要把所有行動電源用於娛樂'],
    checklist: ['手電筒可用', '行動電源已確認', '手機省電模式已開', '冷藏藥品已確認', '夜間動線已清空', '紙本聯絡人可取得', '冰箱開啟規則已告知家人'],
  ...
  },
  {
    ...
    immediateActions: ['固定陽台與戶外物品', '檢查排水孔與排水溝', '檢查窗戶、門縫與可能進水點', '補足飲水與免冷藏食物', '充滿手機與行動電源', '確認動物安置位置'],
    doNotDo: ['不要在強風豪雨時外出檢查', '不要靠近海岸、溪流或淹水道路', '不要把動物安置在可能淹水或過熱處', '不要等到風雨變強才固定戶外物品'],
    checklist: ['戶外物品已固定', '排水孔與排水溝已檢查', '窗戶與可能進水點已檢查', '飲水已補足', '電力已充滿', '現金與文件已準備', '動物安置已完成', '停止外出條件已設定'],
  ...
  },
  {
    ...
    title: '滾動式備糧',
    severity: 4,
    summary: '備糧不是買一批放到過期，而是用平常會吃的食物建立先到期先吃、吃掉後補回的輪替系統。',
    immediateActions: ['列出家中平常會吃的主食、罐頭、乾糧與即食食品', '分成立即可吃、簡單加熱、需要烹調三類', '在庫存標記保存期限與免冷藏狀態', '把最早到期的食物排入本週菜單', '每次吃掉一份後，在採買時補回一份'],
    doNotDo: ['不要只買自己平常不吃的防災食品', '不要沒有保存期限', '不要吃掉後忘記補回', '不要把需要冷藏或複雜烹調的食物算進 72 小時核心'],
    checklist: ['日常會吃的食物已列出', '立即可吃食物已標記', '保存期限已登錄', '先到期先吃規則已建立', '補貨下限已設定'],
    escalation: ['免冷藏食物低於 72 小時最低線', '多項食物 30 天內到期但沒有輪替計畫', '已過期食物仍放在防災庫存內'],
    relatedTools: ['庫存', '報告', '路線']
  },
  {
    id: 'shelter-vs-go-bag',
    filter: 'supply',
    title: '居家避難與撤離包差異',
    severity: 4,
    summary: '居家避難用於住處仍安全時，重點是 7–14 天物資；撤離包用於住處不安全時，重點是能背得動、能快速離開。',
    immediateActions: ['把居家避難儲備與撤離包分開盤點', '居家避難以飲水、食物、常用藥、動物補給與照明為核心', '撤離包以文件、藥品、通訊、最低飲水與機動性為核心', '量測撤離包重量，不把所有物資都塞進包內', '用庫存與撤離包摘要分別檢查缺口'],
    doNotDo: ['不要把所有物資都塞進撤離包', '不要只有撤離包卻沒有居家避難儲備', '不要把過重背包當成安全感'],
    checklist: ['居家 7–14 天物資已盤點', '撤離包重量已計算', '兩者保存期限都已檢查', '動物與慢性病用藥已納入', '家人知道兩者位置與用途'],
    escalation: ['住處不安全或接獲撤離通知', '撤離包負重超過 20%', '居家補給低於 72 小時且外部補給中斷'],
    relatedTools: ['撤離包', '庫存', '計算', '報告']
  },
  {
