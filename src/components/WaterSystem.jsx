...
export default function WaterSystem({ setPage }) {
  ...
  const metrics = useMemo(() => {
    ...
    const potable = calculatePotableStorage(data.storage), nonPotable = calculateNonPotableStorage(data.storage)
    ...
    return { human, animal, drinking, utility, total: drinking + utility, potable, nonPotable, treatmentRequired: calculateTreatmentRequiredStorage(data.storage), drinkingDays, utilityDays, overall: calculateOverallWaterDays(drinkingDays, utilityDays), purification: calculatePurificationCapacity(data.treatments), score, status: getWaterStatus(score), recommendations: generateWaterRecommendations(data) }
  }, [data])
